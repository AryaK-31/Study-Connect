        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-4">
              {user && <ConnectionRequests onConnectionAccepted={() => {}} />}
              <button
                onClick={toggleTheme}
                className={cn(
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.graphQLErrors && err.graphQLErrors.length > 0) {
        errorMessage = String(err.graphQLErrors[0].message);
      } else if (err.message) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center p-4 relative bg-gray-50 dark:bg-gray-900">
      <AnimatePresence>
        {toast && (
          <motion.div
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              type="email"
              required
              className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-colors bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-gray-100"
...
function ProfileSetup({ initialData }) {
  ...
  const handleInterestChange = (val) => {
    ...
    if (existing) {
      setInterests(
        interests.filter((i) => i.toLowerCase().trim() !== lowerVal),
      );
    } else {
  ...
  const addCustomInterest = () => {
    ...
    if (
      customInterest &&
      !interests.some((i) => i.toLowerCase().trim() === lowerVal)
    ) {
  ...
  const handleSubmit = async () => {
    if (
      ...
      (!initialData?.contactNumber && !contactNumber)
    ) {
      alert(
        initialData?.contactNumber
    ...
    try {
      await updateProfile({
        variables: { interests, availability, contactNumber },
      });
      await client.resetStore();
  ...
  const handleDeleteProfile = async () => {
    if (
      !window.confirm(
        "Are you sure you want to delete your profile? This will also delete all sessions you created. This action cannot be undone.",
      )
    )
      return;
    try {
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            ...
            {interests.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {interests.map((i, idx) => (
                  <span
                    key={`${i}-${idx}`}
                    className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 px-3 py-1 rounded-md text-xs font-semibold flex items-center gap-1"
                  >
                    {i}
                    <button
                      onClick={() =>
                        setInterests(
                          interests.filter((_, xIdx) => xIdx !== idx),
                        )
                      }
          ...
          {!initialData?.contactNumber && (
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                value={contactNumber}
          ...
          <div className="space-y-4 pt-4">
            ...
            {initialData && (
              <button
                ...
                className="w-full bg-white dark:bg-gray-700 text-red-500 dark:text-red-400 border border-red-100 dark:border-red-900 py-3 rounded-xl font-semibold hover:bg-red-50 dark:hover:bg-red-900/20 transition-all"
              >
                Delete Account
              </button>
            )}
...
function Dashboard({ currentUser }) {
  ...
  // Compute IDs from query data
  const sentRequestIds = sentRequestsData?.sentRequests?.map((r) => r.id) ?? [];
  const pendingRequestIds =
    sentRequestsData?.pendingRequests?.map((r) => r.id) ?? [];
  const connectedIds = connectionsData?.myConnections?.map((c) => c.id) ?? [];
  ...
  const handleConnect = async (userId) => {
    ...
    if (pendingRequestIds.includes(userId)) {
      setToast({
        message:
          "This user has already sent you a connection request. Check your requests!",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      // Refetch connections and sent requests to update UI
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12 relative">
      ...
      {/* Mobile Tab Switcher */}
      <div className="md:hidden flex p-1 bg-gray-100 dark:bg-gray-800 rounded-xl mb-8">
        <button
      ...
      <section className={cn(activeTab !== "interests" && "hidden md:block")}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {student.interests.map((i, idx) => (
                      <span
                        key={`${i}-${idx}`}
                        className="bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider"
                      >
                        {i}
                      </span>
                    ))}
          ...
          {similarData?.similarStudents?.length === 0 && (
            <div className="col-span-full p-12 text-center bg-gray-50 dark:bg-gray-800 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
              <p className="text-gray-500 dark:text-gray-400">
                No similar students found yet. Try updating your interests!
              </p>
            </div>
          )}
        </div>
      ...
      <section className={cn(activeTab !== "sessions" && "hidden md:block")}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sessionsData?.sessions
            ?.filter((session) => session && session.creator)
            ?.map((session) => (
              <div
                key={session.id}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <button
                ...
                className="absolute top-4 right-4 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 text-2xl"
              >
                ×
              </button>
              ...
              <form onSubmit={handleCreateSession} className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    className="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
        ...
        {selectedUser && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-sm relative text-center"
            >
              <button
              ...
              <div className="flex flex-col gap-3">
                {selectedUser.id !== currentUser?.id &&
                  (() => {
                    const isConnected = connectedIds.includes(selectedUser.id);
                    const hasSentRequest = sentRequestIds.includes(
                      selectedUser.id,
                    );

                    if (isConnected) {
                      return (
                        <div className="w-full bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 py-3 rounded-xl font-bold flex items-center justify-center gap-2">
                    ...
                    if (hasSentRequest) {
                      return (
                        <div className="w-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 py-3 rounded-xl font-bold flex items-center justify-center gap-2">
                          <Check className="w-5 h-5" />
                          Request Pending
                        </div>
                      );
                ...
                {selectedUser.id !== currentUser?.id &&
                  (() => {
                    const connectedIds =
                      connectionsData?.myConnections?.map((c) => c.id) ?? [];
                    const isConnected = connectedIds.includes(selectedUser.id);
                    return (
                      <button
                        onClick={() => {
...
function MainApp() {
  ...
  useEffect(() => {
    ...
    const handleConnectionRequest = (data) => {
      showError(`New connection request from ${data.fromUserName}!`);
    };

    const handleConnectionAccepted = (data) => {
      showError(`${data.byUserName} accepted your connection request!`);
    };
  ...
  }, [user, location.pathname, showError]);
  ...
  if (loading && token)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-gray-900">
        <div className="w-12 h-12 border-4 border-t-transparent rounded-full animate-spin mb-4 border-indigo-600 dark:border-indigo-400"></div>
